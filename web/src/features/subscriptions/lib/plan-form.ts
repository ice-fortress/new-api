...
export function getPlanFormSchema(t: TFunction) {
  return z.object({
    ...
    billing_groups: z.array(z.string()),
...
export function planToFormValues(plan: SubscriptionPlan): PlanFormValues {
  return {
    ...
    // Older plans only have a single group; an explicit empty list clears it.
    billing_groups:
      plan.billing_groups ?? (plan.billing_group ? [plan.billing_group] : []),
...
export function formValuesToPlanPayload(values: PlanFormValues): PlanPayload {
  return {
    plan: {
      ...
      billing_groups: values.billing_groups,
